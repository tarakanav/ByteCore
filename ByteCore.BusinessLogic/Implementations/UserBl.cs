    ...
    public class UserBl : IUserBl
    {
            ...
            string userAgent = null)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                if (db.Users.Any(u => u.Email == email || u.Name == name))
                {
        ...
        public async Task<User> UpdateUserAsync(string currentEmail, User updatedUser)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                if (!user.Email.Equals(updatedUser.Email, StringComparison.OrdinalIgnoreCase) &&
                    db.Users.Any(u => u.Email == updatedUser.Email && u.Id != user.Id))
                {
                    throw new InvalidOperationException("User with this email already exists.");
                }

                if (db.Users.Any(u => u.Name == updatedUser.Name && u.Id != user.Id))
                {
        ...
        public async Task<HttpCookie> GetUserCookieAsync(string email, bool rememberMe = false)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                var session = await db.UserSessions.FirstOrDefaultAsync(x => x.User.Email == email);
                ...
                foreach (var user in users)
                {
                    if (!IsValidEmail(user.Email))
                    {
                        throw new InvalidOperationException(
                            $"Invalid email format for user with ID: {user.Id} or Name: {user.Name}.");
                    }
                    ...
                    if (existingUser != null)
                    {
                        if (!existingUser.Email.Equals(user.Email, StringComparison.OrdinalIgnoreCase) &&
                            db.Users.Any(u => u.Email == user.Email && u.Id != user.Id))
                        {
                            throw new InvalidOperationException(
                                $"Cannot update user with ID: {user.Id}. The email '{user.Email}' is already in use by another user.");
                        }

                        if (db.Users.Any(u => u.Name == user.Name && u.Id != user.Id))
                        {
                            throw new InvalidOperationException(
                                $"Cannot update user with ID: {user.Id}. Another user already has this name.");
                        }

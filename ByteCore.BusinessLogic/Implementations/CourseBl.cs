    ...
    public class CourseBl : ICourseBl
    {
        ...
        public Course GetCourse(int id)
        {
            using (var db = new ApplicationDbContext())
            {
                var course = db.Courses
                    ...
                    .Include(x =>
                        x.Chapters.Select(chapter => chapter.Sections.Select(section => section.Quiz.Questions)))
                    .Include(x => x.Chapters.Select(chapter =>
                        chapter.Sections.Select(section =>
                            section.Quiz.Questions.Select(question => question.Options))))
                                 ...
                                 .SelectMany(chapter => chapter.Sections))
                    {
                        switch (section.Type)
                        {
                            ...
                            case SectionType.Quiz:
                                var quiz = await db.Quizzes.FirstOrDefaultAsync(x => x.Id == section.QuizId);
                                if (quiz == null)
                                {
        ...
        public async Task DeleteCourseAsync(int id)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                if (allSections.Any())
                    db.Set<Section>().RemoveRange(allSections);
        ...
        public IEnumerable<int> GetEnrollmentCountByDate(DateTime fromDate, DateTime toDate)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                var enrollsByDate = db.UserCourses
